...
class QuizzesFilterServiceTest < ActiveSupport::TestCase
  ...
  def test_should_return_quizzes_matching_search_term
    filtered_quizzes = Api::V1::Admin::QuizzesFilterService.new(Quiz.all, { search: "Ruby" }).process!
    assert_equal 1, filtered_quizzes.quizzes.count
    assert_equal "Ruby Basics", filtered_quizzes.quizzes.first.name
  end

  def test_should_return_empty_when_search_term_does_not_match
    filtered_quizzes = Api::V1::Admin::QuizzesFilterService.new(Quiz.all, { search: "Python" }).process!
